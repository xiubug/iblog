...
### 背景
众所众知，React 在进行组件渲染时，从 setState 开始到渲染完成整个过程 JS 是不受控制地执行（想要手动调度）。在这样的条件下，如果需要渲染的组件是一个很大、层级很深的组件，js 执行占据浏览器主线程时间就会过长，恰好阻塞样式计算、界面布局以及许多情况下的绘制，这样就会导致页面响应以及动画、手势等应用体验变差。
